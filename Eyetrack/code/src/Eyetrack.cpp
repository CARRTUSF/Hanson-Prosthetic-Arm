...
namespace {
	...
	bool updated = false;
	int state = 0;
	glm::vec3 posA, posB, eyeFixation, eyePosition, textPositionTrue, EyeFixation_prismPosition;
...
bool Eyetrack::updateLoop(float fDelta) {
	...
	//if (updated) {
		...
		if (state == 0){			
			...
			//update the A position while in scan mode
			posA = EyeFixation_prismPosition;
		}
		else if (state == 4){
			//update the B position while in scan mode
